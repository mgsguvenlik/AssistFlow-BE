CREATE OR ALTER PROCEDURE [qnb].[usp_ReportSearchQnb_MultiServiceTypes]
    @Page                int             = 1,
    @PageSize            int             = 50,
...
WITH base AS (
    ...
    FROM qnb.QnbWorkFlow             wf WITH (NOLOCK)
    JOIN qnb.QnbServicesRequest      sr WITH (NOLOCK) ON sr.RequestNo = wf.RequestNo
    LEFT JOIN qnb.QnbWorkFlowStep    ws WITH (NOLOCK) ON ws.Id = wf.CurrentStepId
    OUTER APPLY (
            ...
            FROM [qnb].[QnbServicesRequestServiceTypes] selected
            JOIN dbo.ServiceType lookup ON lookup.Id = selected.ServiceTypeId
            WHERE selected.[QnbServicesRequestId] = sr.Id
        ) st
...
';
    ...
    IF @ServiceTypeId IS NOT NULL
        SET @sql += N'  AND EXISTS (SELECT 1 FROM [qnb].[QnbServicesRequestServiceTypes] selected WHERE selected.[QnbServicesRequestId] = sr.Id AND selected.ServiceTypeId = @ServiceTypeId) ';
    ...
    IF @TechStatusesCsv IS NOT NULL
        SET @sql += N'  AND EXISTS (
               SELECT 1 FROM qnb.QnbTechnicalService ts WITH (NOLOCK)
    ...
    IF @PricingStatusesCsv IS NOT NULL
        SET @sql += N'  AND EXISTS (
               SELECT 1 FROM qnb.QnbPricing pr WITH (NOLOCK)
        ...
        SET @sql += N'
          AND (
               (@HasImages = 1 AND EXISTS (
                    SELECT 1
                    FROM qnb.QnbTechnicalService t WITH (NOLOCK)
                    LEFT JOIN qnb.QnbTechnicalServiceImage si WITH (NOLOCK) ON si.QnbTechnicalServiceId = t.Id
                    LEFT JOIN qnb.QnbTechnicalServiceFormImage fi WITH (NOLOCK) ON fi.QnbTechnicalServiceId = t.Id
