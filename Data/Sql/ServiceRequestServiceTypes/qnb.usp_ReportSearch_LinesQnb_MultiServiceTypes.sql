CREATE OR ALTER PROCEDURE [qnb].[usp_ReportSearch_LinesQnb_MultiServiceTypes]
    @Page                int             = 1,
    @PageSize            int             = 50,
...
BEGIN
    ...
    ;WITH base AS (
        SELECT
            ...
            sr.ServicesDate,
            sr.ServiceTypeId,
            sr.QnbServiceTrackNo AS ServiceOracleNo,
        ...
        JOIN qnb.QnbServicesRequest sr WITH (NOLOCK)
            ON sr.RequestNo = wf.RequestNo
        LEFT JOIN qnb.QnbWorkFlowStep ws WITH (NOLOCK)
        ...
        LEFT JOIN qnb.QnbFinalApproval fa0 WITH (NOLOCK)
            ON fa0.RequestNo = wf.RequestNo
        WHERE
            wf.IsDeleted = 0
            AND (@ServiceTypeId IS NULL OR EXISTS (SELECT 1 FROM [qnb].[QnbServicesRequestServiceTypes] selected WHERE selected.[QnbServicesRequestId] = sr.Id AND selected.ServiceTypeId = @ServiceTypeId))
            AND (
                @FinalStatusesCsv IS NULL
                OR EXISTS (
                    SELECT 1
                    FROM qnb.QnbFinalApproval fa WITH (NOLOCK)
            ...
            AND (
                ...
                OR (
                    ...
                    AND EXISTS (
                        ...
                        LEFT JOIN qnb.QnbTechnicalServiceImage si WITH (NOLOCK)
                            ON si.QnbTechnicalServiceId = t.Id
                        LEFT JOIN qnb.QnbTechnicalServiceFormImage fi WITH (NOLOCK)
                            ON fi.QnbTechnicalServiceId = t.Id
    ...
    lines AS (
        ...
        FROM base b
        JOIN qnb.QnbServicesRequestProduct l WITH (NOLOCK)
        ...
        LEFT JOIN qnb.QnbTechnicalService ts WITH (NOLOCK)
        ...
        WHERE
            ...
            AND (@ProductCode IS NULL OR p.ProductCode LIKE N'%' + @ProductCode + N'%')
    ),
    lines_calc AS (
        SELECT
            l.*,
