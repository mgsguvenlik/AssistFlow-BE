CREATE OR ALTER PROCEDURE [dbo].[usp_ReportSearch_MultiServiceTypes]
    @Page                int             = 1,
    @PageSize            int             = 50,
...
WITH base AS (
    ...
    JOIN dbo.ServicesRequests     sr WITH (NOLOCK) ON sr.RequestNo = wf.RequestNo
    LEFT JOIN dbo.WorkFlowSteps   ws WITH (NOLOCK) ON ws.Id = wf.CurrentStepId
    OUTER APPLY (
...
';
    ...
    IF @ServiceTypeId IS NOT NULL
        SET @sql += N'  AND EXISTS (SELECT 1 FROM [dbo].[ServicesRequestServiceTypes] selected WHERE selected.[ServicesRequestId] = sr.Id AND selected.ServiceTypeId = @ServiceTypeId) ';
    ...
    IF @TechStatusesCsv IS NOT NULL
        SET @sql += N'  AND EXISTS (
               SELECT 1 FROM dbo.TechnicalServices ts WITH (NOLOCK)
    ...
    IF @PricingStatusesCsv IS NOT NULL
        SET @sql += N'  AND EXISTS (
               SELECT 1 FROM dbo.Pricings pr WITH (NOLOCK)
        ...
        SET @sql += N'
          AND (
               (@HasImages = 1 AND EXISTS (
                    ...
                    LEFT JOIN dbo.TechnicalServiceImages si WITH (NOLOCK) ON si.TechnicalServiceId = t.Id
                    LEFT JOIN dbo.TechnicalServiceFormImages fi WITH (NOLOCK) ON fi.TechnicalServiceId = t.Id
...
totals AS (
    SELECT
        ...
        MAX(CASE
            ...
            END) AS Currency
    FROM dbo.ServicesRequestProducts l WITH (NOLOCK)
