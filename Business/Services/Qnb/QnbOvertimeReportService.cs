...
{
    public class QnbOvertimeReportService : IQnbOvertimeReportService
    {
        private readonly IUnitOfWork _uow;
        private readonly ILogger<QnbOvertimeReportService> _logger;
        private readonly ICurrentUser _currentUser;
        private readonly IWorkingHourPolicyService _workingHourPolicyService;

        public QnbOvertimeReportService(
            IUnitOfWork uow,
            ILogger<QnbOvertimeReportService> logger,
        ...
        public async Task<ResponseModel<QnbTechnicianOvertimeReportDto>> GetTechnicianOvertimeReportAsync(
            long technicianId,
            DateTime startDate,
            DateTime endDate,
            bool includeCustomerDetails = false)
        {
            try
            {
                ...
                if (technician == null)
                    return ResponseModel<QnbTechnicianOvertimeReportDto>.Fail(
                        "Teknisyen bulunamadı.",
                        StatusCode.NotFound);

                // QNB WorkFlow'ları çek
                ...
                foreach (var ts in technicalServices)
                {
                    ...
                    if (overtimeResult.TotalOvertimeHours > 0)
                    {
                        ...
                        {
                            job.QnbCustomerFormId = customer.Id;
                ...
                var report = new QnbTechnicianOvertimeReportDto
                {
                ...
                return ResponseModel<QnbTechnicianOvertimeReportDto>.Success(report);
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "QNB - GetTechnicianOvertimeReportAsync - TechnicianId: {TechnicianId}", technicianId);
                return ResponseModel<QnbTechnicianOvertimeReportDto>.Fail(
        ...
        public async Task<ResponseModel<QnbAllTechniciansOvertimeSummaryDto>> GetAllTechniciansOvertimeSummaryAsync(
            DateTime startDate,
            DateTime endDate)
        {
            try
            {
                ...
                var summary = new QnbAllTechniciansOvertimeSummaryDto
                {
                    StartDate = startDate,
                    EndDate = endDate,
                    TotalOvertimeHours = Math.Round(totalOvertimeHours, 2),
                ...
                return ResponseModel<QnbAllTechniciansOvertimeSummaryDto>.Success(summary);
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "QNB - GetAllTechniciansOvertimeSummaryAsync");
                return ResponseModel<QnbAllTechniciansOvertimeSummaryDto>.Fail(
            ...
            DateTime endDate)
        {
            try
            {
                ResponseModel<QnbTechnicianOvertimeReportDto>? singleReport = null;
                ResponseModel<QnbAllTechniciansOvertimeSummaryDto>? summaryReport = null;
                ...
                {
                    var worksheet = workbook.Worksheets.Add("QNB Fazla Mesai Raporu");
                ...
                {
                    var worksheet = workbook.Worksheets.Add("QNB Fazla Mesai Özeti");
            ...
            {
                _logger.LogError(ex, "QNB - ExportOvertimeReportToExcelAsync");
        ...
        // Hesaplama metodları (YKB ile birebir aynı)
