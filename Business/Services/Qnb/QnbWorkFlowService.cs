    ...
    public partial class QnbWorkFlowService : IQnbWorkFlowService
    {
        ...
        public async Task<ResponseModel<QnbServicesRequestGetDto>> CreateRequestAsync(QnbServicesRequestCreateDto dto)
        {
            try
            {
                ...
                if (serviceTypeError is not null)
                    return ResponseModel<QnbServicesRequestGetDto>.Fail(serviceTypeError, StatusCode.BadRequest);
                ...
                request.ServiceTypeRelations = serviceTypeIds
                    .Select(id => new QnbServicesRequestServiceType { ServiceTypeId = id }).ToList();
        ...
        public async Task<ResponseModel<QnbPricingGetDto>> ApprovePricing(QnbPricingUpdateDto dto)
        {
            ...
            catch (InvalidDataException ex)
            {
                ...
                _logger.LogWarning(
                    ex,
                    "QNB ApprovePricing dosya doğrulama hatası. " +
                    "RequestNo: {RequestNo}",
                    dto?.RequestNo);

                return ResponseModel<QnbPricingGetDto>.Fail(
                    ex.Message,
                    StatusCode.BadRequest);
            }
            catch (Exception ex)
        ...
        public async Task<ResponseModel> DeleteTechnicalServiceImageAsync(long id, TechnicalServiceImageType type, CancellationToken cancellationToken = default)
        {
            try
            {
                ...
                if (type == TechnicalServiceImageType.Service)
                {
                    ...
                    await _uow.Repository
                        .HardDeleteAsync<QnbTechnicalServiceImage, long>(image);
                }
                else
                {
                    var image = await _uow.Repository
                    ...
                    await _uow.Repository
                        .HardDeleteAsync<QnbTechnicalServiceFormImage, long>(image);
                }
            ...
            catch (Exception ex)
            {
                _logger.LogError(
                    ex,
                    "QNB teknik servis resmi silinirken hata oluştu. Id: {Id}, Type: {Type}",
                    id,
                    type);
        ...
        public async Task<PagedResult<QnbWorkFlowReportListItemDto>> GetReportsAsync(QnbReportQueryParams q)
        {
            ...
            try
            {
                ...
                var rows = await conn.QueryAsync<ReportRowDto>(new CommandDefinition(
                    "qnb.usp_ReportSearchQnb_MultiServiceTypes",
                    p,
                    transaction: efTx,
                ...
                var reportTypes = await _ctx.Set<QnbServicesRequestServiceType>().AsNoTracking()
                    .Where(x => reportRequestNos.Contains(x.QnbServicesRequest.RequestNo))
                    .Select(x => new { x.QnbServicesRequest.RequestNo, Type = new Model.Dtos.ServiceType.ServiceTypeGetDto
        ...
        public async Task<PagedResult<QnbWorkFlowReportLineDto>> GetReportLinesAsync(QnbReportQueryParams q)
        {
            ...
            try
            {
                ...
                var rows = await conn.QueryAsync<QnbReportLineRowDto>(new CommandDefinition(
                    "qnb.usp_ReportSearch_LinesQnb_MultiServiceTypes",
                    p,
                    transaction: efTx,
                    commandType: CommandType.StoredProcedure,
                    commandTimeout: 60
        ...
        private void SyncServiceTypes(QnbServicesRequest request, IReadOnlyCollection<long> ids)
        {
            ...
            foreach (var id in ids.Except(request.ServiceTypeRelations.Select(x => x.ServiceTypeId)).ToList())
                request.ServiceTypeRelations.Add(new QnbServicesRequestServiceType { ServiceTypeId = id });
        }

        private Task<List<Model.Dtos.ServiceType.ServiceTypeGetDto>> GetServiceTypesAsync(string requestNo)
            => _ctx.Set<QnbServicesRequestServiceType>().AsNoTracking()
                .Where(x => x.QnbServicesRequest.RequestNo == requestNo)
        ...
        private async Task ArchiveWorkflowAsync(string requestNo, string archiveReason, CancellationToken ct = default)
        {
            ...
            var archive = new QnbWorkFlowArchive
            {
                ...
                QnbWorkflowAttachmentsJson = JsonConvert.SerializeObject(workflowAttachments)
            };
        ...
        private QnbWorkFlowArchiveDetailDto BuildArchiveDetailDto(QnbWorkFlowArchive archive)
        {
            ...
            List<QnbWorkflowAttachment> attachments = new();
            ...
            var attachmentDtos = attachments
                    .Select(x =>
                    {
                        ...
                        return new QnbWorkflowAttachmentGetDto
                        {
                            ...
                            Url = string.IsNullOrWhiteSpace(baseUrl) ? relativeUrl : $"{baseUrl}{relativeUrl}"
                        };
                    })
                     .ToList();
