...
namespace Business.Services.Ekb
{
    public class EkbWorkFlowService : IEkbWorkFlowService
    {
        private readonly IUnitOfWork _uow;
        private readonly TypeAdapterConfig _config;
        private readonly IActivationRecordService _activationRecord;
        private readonly ILogger<EkbWorkFlowService> _logger;
        ...
        public EkbWorkFlowService(IUnitOfWork uow, TypeAdapterConfig config, IAuthService authService, IActivationRecordService activationRecord,
            ILogger<EkbWorkFlowService> logger, IMailPushService mailPush, ICurrentUser currentUser, AppDataContext ctx, INotificationService notification, IMenuService menuService, IManitouApiService manitouApiService, IFileStorage fileStorage)
        {
            _uow = uow;
            _config = config;
        ...
        public async Task<ResponseModel<EkbCustomerFormGetDto>> CreateCustomerForm(EkbCustomerFormCreateDto dto)
        {
            try
            {
                #region Validasyon/Kontroller
                // Başlangıç WorkFlowStep'i Bul
                var targetStep = await _uow.Repository.GetQueryable<EkbWorkFlowStep>()
                    .AsNoTracking()
                    .FirstOrDefaultAsync(s => s.Code == "SR"); // Örn: 'SR' (Services Request) kodu ile başlangıç adımı

                if (targetStep is null)
                    return ResponseModel<EkbCustomerFormGetDto>.Fail("İş akışı hedef adımı (SR) tanımlı değil.", StatusCode.BadRequest);

                // RequestNo yoksa üret
                if (string.IsNullOrWhiteSpace(dto.RequestNo))
                {
                    var rn = await GetRequestNoAsync("EKB");
                    if (!rn.IsSuccess)
                        return ResponseModel<EkbCustomerFormGetDto>.Fail(rn.Message, rn.StatusCode);
                ...
                if (exists)
                    return ResponseModel<EkbCustomerFormGetDto>.Fail("Aynı akış numarasi ile başka bir kayıt zaten var. Lütfen tekrar deneyiniz", StatusCode.Conflict);


                var customerExist = await _uow.Repository.GetQueryable<Customer>().AsNoTracking().AnyAsync(c => c.Id == dto.CustomerId);
                if (!customerExist)
                    return ResponseModel<EkbCustomerFormGetDto>.Fail("Müşteri bulunamadı.", StatusCode.Conflict);
                ...
                if (!customerApproverExist)
                    return ResponseModel<EkbCustomerFormGetDto>.Fail("Müşteri yetkilisi bulunamadı.", StatusCode.Conflict);

                var (ekbWorkOrderTypeIds, ekbWorkOrderTypeError) = await ValidateWorkOrderTypeIdsAsync(dto.WorkOrderTypeIds);
                if (ekbWorkOrderTypeError is not null)
                    return ResponseModel<EkbCustomerFormGetDto>.Fail(ekbWorkOrderTypeError, StatusCode.BadRequest);
                ...
                var request = customerForm.Adapt<EkbServicesRequest>(_config);
                ...
                request.EkbServicesRequestWorkOrderTypes = ekbWorkOrderTypeIds
                    .Select(wotId => new EkbServicesRequestWorkOrderType { WorkOrderTypeId = wotId })
                    .ToList();
                await _uow.Repository.AddAsync(request);
                #endregion 

                #region Hareket Kaydı
                await _activationRecord.LogEkbAsync(
            ...
            catch (Exception ex)
            {
                ...
                return ResponseModel<EkbCustomerFormGetDto>.Fail($"CreateCustomerForm Oluşturma sırasında hata: {ex.Message}", StatusCode.Error);
            }
        }
        //1 Servis Talebi güncelleme adımı :
        public async Task<ResponseModel<EkbServicesRequestGetDto>> UpdateServiceRequestAsync(EkbServicesRequestUpdateDto dto)
        {
            var entity = await _uow.Repository.GetSingleAsync<EkbServicesRequest>(
                false,
                x => x.RequestNo == dto.RequestNo,
                includeExpression: RequestIncludes());

            if (entity is null)
                return ResponseModel<EkbServicesRequestGetDto>.Fail("Kayıt bulunamadı.", StatusCode.NotFound);
            ...
            if (wf is null)
                return ResponseModel<EkbServicesRequestGetDto>.Fail("İlgili akış kaydı bulunamadı.", StatusCode.NotFound);
            ...
            var existingProducts = await _uow.Repository
                .GetMultipleAsync<EkbServicesRequestProduct>(
            ...
            if (dto.Products is not null)
            {
                ...
                existingProducts ??= new List<EkbServicesRequestProduct>();
                ...
                {
                    var entityProd = new EkbServicesRequestProduct
                    {
                        RequestNo = dto.RequestNo,
            ...
            {
                var ekbSrEntity = await _uow.Repository.GetQueryable<EkbServicesRequest>()
                    .Include(x => x.EkbServicesRequestWorkOrderTypes)
                    .FirstOrDefaultAsync(x => x.RequestNo == dto.RequestNo);

                if (ekbSrEntity is not null)
                {
                    var (validatedWotIds, wotError) = await ValidateWorkOrderTypeIdsAsync(dto.WorkOrderTypeIds);
                    if (wotError is null)
                        SyncEkbWorkOrderTypes(ekbSrEntity, validatedWotIds);
                }
            }
            #endregion
        ...
        public async Task<ResponseModel<EkbWarehouseGetDto>> SendWarehouseAsync(EkbSendWarehouseDto dto)
        {
            try
            {
                #region Validasyon/Kontroller
                ...
                var request = await _uow.Repository
                    .GetQueryable<EkbServicesRequest>()
                    .FirstOrDefaultAsync(x => x.RequestNo == dto.RequestNo);

                if (request is null)
                    return ResponseModel<EkbWarehouseGetDto>.Fail("Servis talebi bulunamadı.", StatusCode.NotFound);

                if (request.ServicesRequestStatus == ServicesRequestStatus.WarehouseSubmitted)
                    return ResponseModel<EkbWarehouseGetDto>.Fail("Bu talep zaten depoya gönderilmiş.", StatusCode.Conflict);


                var product = await _uow.Repository.GetQueryable<EkbServicesRequestProduct>(x => x.RequestNo == dto.RequestNo).ToListAsync();
                if (product is null || product.Count == 0)
                    return ResponseModel<EkbWarehouseGetDto>.Fail("Bu talep için kayıtlı ürün bulunamadı. Depoya gönderim için en az bir ürün eklenmiş olmalıdır.", StatusCode.BadRequest);
                ...
                if (wf is null)
                    return ResponseModel<EkbWarehouseGetDto>.Fail("İlg  kaydı bulunamadı.", StatusCode.NotFound);

                if (wf.WorkFlowStatus == WorkFlowStatus.Cancelled)
                    return ResponseModel<EkbWarehouseGetDto>.Fail("İlgili akış iptal edilmiş.", StatusCode.NotFound);

                if (wf.WorkFlowStatus == WorkFlowStatus.Complated)
                    return ResponseModel<EkbWarehouseGetDto>.Fail("İlgili akış iptal tamamlanmış.", StatusCode.NotFound);
            ...
            catch (Exception ex)
            {
                ...
                return ResponseModel<EkbWarehouseGetDto>.Fail($"Depo gönderim sırasında hata: {ex.Message}", StatusCode.Error);
            }
        }

        //2.2 Depo Teslimatı ve Teknik servise Gönderim (Ürün var ise)
        public async Task<ResponseModel<EkbWarehouseGetDto>> CompleteDeliveryAsync(EkbCompleteDeliveryDto dto)
        {
            ...
            try
            {
                ...
                if (wf is null)
                    return ResponseModel<EkbWarehouseGetDto>.Fail("İlgili akış kaydı bulunamadı.", StatusCode.NotFound);
                ...
                if (warehouse is null)
                    return ResponseModel<EkbWarehouseGetDto>.Fail("Depo kaydı bulunamadı.", StatusCode.NotFound);


                // 🔹 WorkFlow güncelle
                var targetStep = await _uow.Repository
                    .GetQueryable<EkbWorkFlowStep>()
                    .AsNoTracking()
                ...
                if (targetStep is null)
                    return ResponseModel<EkbWarehouseGetDto>.Fail("WorkFlowStep içinde 'Teknik Servis' statüsü tanımlı değil.", StatusCode.BadRequest);
                ...
                {
                    technicalService = new EkbTechnicalService
                    {
                        RequestNo = dto.RequestNo,
                ...
                {
                    var newEntity = new EkbServicesRequestProduct
                    {
            ...
            catch (Exception ex)
            {
                ...
                return ResponseModel<EkbWarehouseGetDto>.Fail($" Depo Teslimatı  sırasında hata: {ex.Message}", StatusCode.Error);
            }

        }

        //2.3 Teknik Servis Gönderim  (Ürün yok ise)
        public async Task<ResponseModel<EkbTechnicalServiceGetDto>> SendTechnicalServiceAsync(EkbSendTechnicalServiceDto dto)
        {
            try
            {
                ...
                if (wf is null)
                    return ResponseModel<EkbTechnicalServiceGetDto>.Fail("İlg  kaydı bulunamadı.", StatusCode.NotFound);

                if (wf.WorkFlowStatus == WorkFlowStatus.Cancelled)
                    return ResponseModel<EkbTechnicalServiceGetDto>.Fail("İlgili akış iptal edilmiş.", StatusCode.NotFound);

                if (wf.WorkFlowStatus == WorkFlowStatus.Complated)
                    return ResponseModel<EkbTechnicalServiceGetDto>.Fail("İlgili akış iptal tamamlanmış.", StatusCode.NotFound);
                ...
                if (request is null)
                    return ResponseModel<EkbTechnicalServiceGetDto>.Fail("Servis talebi bulunamadı.", StatusCode.NotFound);
                ...
                if (targetStep is null)
                    return ResponseModel<EkbTechnicalServiceGetDto>.Fail("Hedef iş akışı adımı (TS) tanımlı değil.", StatusCode.BadRequest);
            ...
            catch (Exception ex)
            {
                ...
                return ResponseModel<EkbTechnicalServiceGetDto>.Fail($"Teknik Servis Gönderim  sırasında hata: {ex.Message}", StatusCode.Error);
            }

        }

        // 3 Teknik Servis Servisi Başlatma 
        public async Task<ResponseModel<EkbTechnicalServiceGetDto>> StartService(EkbStartTechnicalServiceDto dto)
        {
            ...
            try
            {
                ...
                if (customer is null)
                    return ResponseModel<EkbTechnicalServiceGetDto>.Fail("İlgili müşteri kaydı bulunamadı.", StatusCode.NotFound);
                ...
                if (technicalService is null)
                    return ResponseModel<EkbTechnicalServiceGetDto>.Fail("İlgili teknik servis kaydı bulunamadı.", StatusCode.NotFound);

                if (technicalService.ServicesStatus == TechnicalServiceStatus.InProgress)
                    return ResponseModel<EkbTechnicalServiceGetDto>.Fail("Teknik servis zaten başlatılmış", StatusCode.Conflict);
                ...
                if (technicalService.IsLocationCheckRequired) //Lokasyon kontrolü gerekli ise
                {
                    ...
                    {
                        return ResponseModel<EkbTechnicalServiceGetDto>.Fail("Lokasyon bilgileri gönderilmemiş.", StatusCode.InvalidCustomerLocation);
                    }
                    else
                    {
                        ...
                        if (!locationResult.IsSuccess)
                        {
                            ...
                            return ResponseModel<EkbTechnicalServiceGetDto>.Fail(locationResult.Message, locationResult.StatusCode);
                        }
                    }
                }
            ...
            catch (Exception ex)
            {
                ...
                return ResponseModel<EkbTechnicalServiceGetDto>.Fail($" Teknik Servis Servisi Başlatma   sırasında hata: {ex.Message}", StatusCode.Error);
            }

        }

        // 3.1 Teknik Servis Servisi Tamamlama  ve Fiyatlamaya gönderimi
        public async Task<ResponseModel<EkbTechnicalServiceGetDto>> FinishService(EkbFinishTechnicalServiceDto dto)
        {
            try
            {
                ...
                if (targetStep is null)
                    return ResponseModel<EkbTechnicalServiceGetDto>.Fail("Hedef iş akışı adımı (PRC) tanımlı değil.", StatusCode.BadRequest);
                ...
                if (isTestEnabled)
                {
                    var activeWorkingExists = await _uow.Repository
                        .GetQueryable<EkbTechnicalServiceWorkSession>()
                        .AsNoTracking()
                        .AnyAsync(x =>
                    ...
                    {
                        return ResponseModel<EkbTechnicalServiceGetDto>.Fail(
                ...
                .GetQueryable<EkbPricing>()
                .FirstOrDefaultAsync(x => x.RequestNo == dto.RequestNo);

                if (pricing is null)
                {
                    pricing = new EkbPricing()
                    {
                        RequestNo = dto.RequestNo,
                ...
                var toAddImages = new List<EkbTechnicalServiceImage>();
                var toAddFormImages = new List<EkbTechnicalServiceFormImage>();
                ...
                try
                {
                    if (dto.ServiceImages is not null)
                    {
                        foreach (var f in dto.ServiceImages)
                        {
                            ...
                            toAddImages.Add(new EkbTechnicalServiceImage
                            {
                                EkbTechnicalServiceId = technicalService.Id,
                                Url = url,
                    ...
                    if (dto.FormImages is not null)
                    {
                        foreach (var f in dto.FormImages)
                        {
                            var url = await SaveAsync(f, CancellationToken.None);
                            if (url is null) continue;
                            toAddFormImages.Add(new EkbTechnicalServiceFormImage
                            {
                                EkbTechnicalServiceId = technicalService.Id,
                                Url = url,
                                Caption = "Form Resmi"
                ...
                // Dictionary ile hızlı karşılaştırma
                var deliveredDict = dto?.Products?.ToDictionary(x => x.ProductId, x => x) ?? new Dictionary<long, EkbServicesRequestProductCreateDto>();
                // 1️ Güncelle veya Sil (mevcut ürünler üzerinden)
                foreach (var existing in existingProducts)
                {
                    if (deliveredDict.TryGetValue(existing.ProductId, out var delivered))
            ...
            catch (Exception ex)
            {
                ...
                return ResponseModel<EkbTechnicalServiceGetDto>.Fail($" Teknik Servis Servisi Tamamlama  ve Fiyatlamaya gönderimi   sırasında hata: {ex.Message}", StatusCode.Error);
            }
        ...
        public async Task<ResponseModel<EkbPricingGetDto>> ApprovePricing(EkbPricingUpdateDto dto)
        {
            ...
            try
            {
                ...
                if (wf is null)
                    return ResponseModel<EkbPricingGetDto>.Fail("İlg  kaydı bulunamadı.", StatusCode.NotFound);

                if (wf.WorkFlowStatus == WorkFlowStatus.Cancelled)
                    return ResponseModel<EkbPricingGetDto>.Fail("İlgili akış iptal edilmiş.", StatusCode.NotFound);

                if (wf.WorkFlowStatus == WorkFlowStatus.Complated)
                    return ResponseModel<EkbPricingGetDto>.Fail("İlgili akış iptal tamamlanmış.", StatusCode.NotFound);
                ...
                if (request is null)
                    return ResponseModel<EkbPricingGetDto>.Fail("Servis talebi bulunamadı.", StatusCode.NotFound);
                ...
                if (targetStep is null)
                    return ResponseModel<EkbPricingGetDto>.Fail("Hedef iş akışı adımı (TS) tanımlı değil.", StatusCode.BadRequest);
                ...
                if (pricing is null)
                    return ResponseModel<EkbPricingGetDto>.Fail("Fiyatlama kaydı tanımlı değil.", StatusCode.BadRequest);
                ...
                if (servicesRequest is null)
                    return ResponseModel<EkbPricingGetDto>.Fail("Servis talebi kaydı bulunamadı.", StatusCode.BadRequest);
                ...
                var deliveredDict = dto?.Products?.ToDictionary(x => x.ProductId, x => x)
                                    ?? new Dictionary<long, EkbServicesRequestProductCreateDto>();
                ...
                var finalApproval = await _uow.Repository
                        .GetQueryable<EkbFinalApproval>()
                        .FirstOrDefaultAsync(x => x.RequestNo == dto.RequestNo);
                if (finalApproval is null)
                {
                    finalApproval = new EkbFinalApproval
                    {
                        RequestNo = dto.RequestNo,
                ...
                var captureResult = await EnsurePricesCapturedFromDtoAsync(dto.RequestNo, dto.Products);
                if (!captureResult.IsSuccess)
                    return ResponseModel<EkbPricingGetDto>.Fail(captureResult.Message, captureResult.StatusCode);
                #endregion
            ...
            catch (InvalidDataException ex)
            {
                ...
                return ResponseModel<EkbPricingGetDto>.Fail(
                    ex.Message,
        ...
        public async Task<ResponseModel<EkbFinalApprovalGetDto>> FinalApprovalAsync(EkbFinalApprovalUpdateDto dto)
        {
            WorkflowAttachmentChangeSet? attachmentChangeSet = null;
            var attachmentsCommitted = false;
            ...
            try
            {
                ...
                if (wf is null)
                    return ResponseModel<EkbFinalApprovalGetDto>.Fail("İlgili akış kaydı bulunamadı.", StatusCode.NotFound);

                if (wf.WorkFlowStatus == WorkFlowStatus.Cancelled)
                    return ResponseModel<EkbFinalApprovalGetDto>.Fail("İlgili akış iptal edilmiş.", StatusCode.NotFound);

                if (wf.WorkFlowStatus == WorkFlowStatus.Complated)
                    return ResponseModel<EkbFinalApprovalGetDto>.Fail("İlgili akış tamamlanmış.", StatusCode.NotFound);
                ...
                if (request is null)
                    return ResponseModel<EkbFinalApprovalGetDto>.Fail("Servis talebi bulunamadı.", StatusCode.NotFound);
                ...
                if (targetStep is null)
                    return ResponseModel<EkbFinalApprovalGetDto>.Fail($"Hedef iş akışı adımı {statusCode} tanımlı değil.", StatusCode.BadRequest);
                ...
                if (existsFinalApproval is null)
                    return ResponseModel<EkbFinalApprovalGetDto>.Fail("Kayıt bulunamadı.", StatusCode.BadRequest);

                if (existsFinalApproval.Status == FinalApprovalStatus.CustomerApproval)
                    return ResponseModel<EkbFinalApprovalGetDto>.Fail($"Hedef iş akışı müşteri onayında.", StatusCode.BadRequest);
                ...
                if (isTerminalStatus)
                {
                    ...
                    {
                        return ResponseModel<EkbFinalApprovalGetDto>.Fail(
                ...
                var captureResult = await EnsurePricesCapturedFromDtoAsync(dto.RequestNo, dto.Products);
                if (!captureResult.IsSuccess)
                    return ResponseModel<EkbFinalApprovalGetDto>.Fail(captureResult.Message, captureResult.StatusCode);
                #endregion
            ...
            catch (Exception ex)
            {
                ...
                return ResponseModel<EkbFinalApprovalGetDto>.Fail($"  Kontrol ve Son Onay sırasında hata: {ex.Message}", StatusCode.Error);
            }

        }

        // 6 Müşteri Onayı
        public async Task<ResponseModel<EkbFinalApprovalGetDto>> CustomerAgreementAsync(EkbCustomerAgreementDto dto)
        {
            try
            {
                ...
                if (wf.CurrentStep?.Code != "CAPR")
                    return ResponseModel<EkbFinalApprovalGetDto>.Fail("Bu işlem sadece EKB müşteri onay adımında yapılabilir.", StatusCode.BadRequest);
                ...
                if (finalApproval is null)
                    return ResponseModel<EkbFinalApprovalGetDto>.Fail("FinalApproval kaydı bulunamadı.", StatusCode.NotFound);

                var complatedStep = await _uow.Repository.GetQueryable<EkbWorkFlowStep>()
                    .AsNoTracking()
                    .FirstOrDefaultAsync(s => s.Code == "CMP");

                var cancelledStep = await _uow.Repository.GetQueryable<EkbWorkFlowStep>()
                   .AsNoTracking()
                ...
                if (dto.IsAgreed)
                {
                    ...
                    await _activationRecord.LogEkbAsync(
                        ...
                        fromStepCode: "CAPR",
                        toStepCode: "APR",
                        "EKB tarafından Mutabık Kalındı ve süreç tamamlandı.",
                    ...
                    await _notification.CreateForRolesAsync(
                        new NotificationCreateDto
                        {
                            ...
                            Message = $"EKB son onayı alındı. Müşteri: {request.Customer?.ContactName1 ?? "-"}",
                            RequestNo = dto.RequestNo,
                            FromStepCode = "CAPR",
                            ToStepCode = "APR",
                        },
            ...
            catch (Exception ex)
            {
                ...
                return ResponseModel<EkbFinalApprovalGetDto>.Fail($"EKB müşteri onayı sırasında hata: {ex.Message}", StatusCode.Error);
            }
        }

        //Lokasyon Kontrolü  Ezme Maili 
        public async Task<ResponseModel> RequestLocationOverrideAsync(EkbOverrideLocationCheckDto dto)
        {
        ...
        //--------------------- Customer Form ----------------------------
        public async Task<ResponseModel<EkbCustomerFormGetDto>> GetCustomerFormByRequestNoAsync(string requestNo)
        {
            // 1) Ana kayıt sadece EkbCustomerForm üzerinden alınır.
            var baseDto = await _uow.Repository
                .GetQueryable<EkbCustomerForm>()
                .AsNoTracking()
                .Where(sr => sr.RequestNo == requestNo)
                .Select(sr => new EkbCustomerFormGetDto
                {
                    Id = sr.Id,
                    RequestNo = sr.RequestNo,
                    EkbServiceTrackNo = sr.EkbServiceTrackNo,
                    ...
                    // EkbCustomerForm içinde bu iki kolon yok.
                    // Aşağıda EkbServicesRequest'ten doldurulacak.
                    ServiceTypeId = null,
            ...
            if (baseDto is null)
            {
                return ResponseModel<EkbCustomerFormGetDto>.Fail(
                    "Kayıt bulunamadı.",
                    StatusCode.NotFound
                );
            }
            ...
            // 3) ServicesCostStatus ve ServiceTypeId sadece EkbServicesRequest'ten gelir
            ...
            baseDto.ServicesRequestProducts = await _uow.Repository
                .GetQueryable<EkbServicesRequestProduct>()
                .AsNoTracking()
                .Where(p => p.RequestNo == requestNo)
                .Select(p => new EkbServicesRequestProductGetDto
                {
                    Id = p.Id,
            ...
            baseDto.ReviewLogs = await _uow.Repository
                .GetQueryable<EkbWorkFlowReviewLog>(
                    x => x.RequestNo == requestNo &&
                         (x.FromStepCode == "SR" || x.ToStepCode == "SR")
                )
                .AsNoTracking()
                .OrderByDescending(x => x.CreatedDate)
                .Select(x => new EkbWorkFlowReviewLogDto
                {
                    Id = x.Id,
                    EkbWorkFlowId = x.EkbWorkFlowId,
            ...
            baseDto.WorkOrderTypes = await _uow.Repository
                .GetQueryable<EkbServicesRequestWorkOrderType>()
                .AsNoTracking()
                .Where(x => x.EkbServicesRequest.RequestNo == requestNo)
            ...
            return ResponseModel<EkbCustomerFormGetDto>.Success(baseDto);
        }
        // -------------------- Services Request --------------------
        private static Func<IQueryable<EkbServicesRequest>, IIncludableQueryable<EkbServicesRequest, object>>? RequestIncludes()
            => q => q
                ...
                .Include(x => x.CustomerApprover)
                .Include(x => x.EkbWorkFlowStep);
        public async Task<ResponseModel<PagedResult<EkbServicesRequestGetDto>>> GetRequestsAsync(QueryParams q)
        {
            var me = await _currentUser.GetAsync();
            if (me is null)
                return ResponseModel<PagedResult<EkbServicesRequestGetDto>>.Fail("Kullanıcı bulunamadı.", StatusCode.Unauthorized);
            ...
            IQueryable<EkbWorkFlow> wfBase = _uow.Repository.GetQueryable<EkbWorkFlow>()
                .AsNoTracking()
                .Where(x => !x.IsDeleted);
            ...
            var query = _uow.Repository.GetQueryable<EkbServicesRequest>();
            ...
            var items = await query
                ...
                .Take(pageSize)
                .ProjectToType<EkbServicesRequestGetDto>(_config)
                .ToListAsync();

            return ResponseModel<PagedResult<EkbServicesRequestGetDto>>.Success(
                new PagedResult<EkbServicesRequestGetDto>(items, total, page, pageSize)
            );
        }
        ...
        public async Task<ResponseModel<PagedResult<ActiveCustomerRequestDto>>> GetActiveCustomerRequestsAsync(long customerId, int page, int pageSize)
        {
            ...
            var query = from workflow in _uow.Repository.GetQueryable<EkbWorkFlow>().AsNoTracking()
                        join request in _uow.Repository.GetQueryable<EkbServicesRequest>().AsNoTracking()
        ...
        public async Task<ResponseModel<EkbServicesRequestGetDto>> GetServiceRequestByIdAsync(long id)
        {
            var now = DateTimeOffset.Now;

            // 1) Ana DTO: SR + (WF last) + Customer (warranty türetmeleri)
            var baseDto = await (
                from sr in _uow.Repository.GetQueryable<EkbServicesRequest>().AsNoTracking()
                where sr.Id == id

                // left join: aynı RequestNo’ya sahip ve silinmemiş workflow’lar
                join wf0 in _uow.Repository.GetQueryable<EkbWorkFlow>().AsNoTracking().Where(w => !w.IsDeleted)
                    on sr.RequestNo equals wf0.RequestNo into wfJoin
                from wf in wfJoin
                    .OrderByDescending(x => x.CreatedDate)  // “en güncel” workflow tercih ediliyorsa
                    .Take(1)
                    .DefaultIfEmpty()
                select new EkbServicesRequestGetDto
                {
                    Id = sr.Id,
                    RequestNo = sr.RequestNo,
                    OracleNo = sr.EkbServiceTrackNo,
                    ServicesDate = sr.ServicesDate,
                    PlannedCompletionDate = sr.PlannedCompletionDate,
                    ...
                    WorkFlowStepName = sr.EkbWorkFlowStep != null ? sr.EkbWorkFlowStep.Name : null,
                    CreatedDate = sr.CreatedDate,
                    UpdatedDate = sr.UpdatedDate,
                    CreatedUser = sr.CreatedUser,
                    UpdatedUser = sr.UpdatedUser,
            ...
            baseDto.ServicesRequestProducts = await _uow.Repository
                ...
                .Select(p => new EkbServicesRequestProductGetDto
                {
                    ...
                    /*
                     * Effective Price önceliği:
                     *
                     * 1. CustomerGroupProductPrice
                     * 2. CustomerProductPrice
                     * 3. TenantProductPrice
                     * 4. Product
                     */
                    EffectivePrice =
                        p.Customer.CustomerGroup.GroupProductPrices
            ...
            baseDto.ReviewLogs = await _uow.Repository
                .GetQueryable<EkbWorkFlowReviewLog>(x => x.RequestNo == baseDto.RequestNo && (x.FromStepCode == "SR" || x.ToStepCode == "SR"))
                .AsNoTracking()
                .OrderByDescending(x => x.CreatedDate)
                .Select(x => new EkbWorkFlowReviewLogDto
                {
            ...
            baseDto.WorkOrderTypes = await _uow.Repository
                .GetQueryable<EkbServicesRequestWorkOrderType>()
                .AsNoTracking()
                .Where(x => x.EkbServicesRequest.RequestNo == baseDto.RequestNo)
                .OrderBy(x => x.WorkOrderType.Name)
                .Select(x => new WorkOrderTypeGetDto
                {
                    Id = x.WorkOrderTypeId,
            ...
            return ResponseModel<EkbServicesRequestGetDto>.Success(baseDto);
        }
        public async Task<ResponseModel<EkbServicesRequestGetDto>> GetServiceRequestByRequestNoAsync(string requestNo)
        {
            var now = DateTimeOffset.Now;
            ...
            baseDto.ServicesRequestProducts = await _uow.Repository
                ...
                .Select(p => new EkbServicesRequestProductGetDto
                {
                    ...
                    // Product'ın kendi standart fiyatı
                    ProductPrice =
                        (p.Product != null
                            ? (decimal?)p.Product.Price
                            : null)
                    ...
                    /*
                     * Currency, fiyat hangi kaynaktan geldiyse
                     * kesinlikle aynı kaynaktan alınır.
                     *
                     * Default TRY YOK.
                     */
                    PriceCurrency =
            ...
            // 3) Review logs (tek bağımsız sorgu — SR adımıyla sınırlı)
            baseDto.ReviewLogs = await _uow.Repository
                .GetQueryable<EkbWorkFlowReviewLog>(x => x.RequestNo == requestNo && (x.FromStepCode == "SR" || x.ToStepCode == "SR"))
                .AsNoTracking()
                .OrderByDescending(x => x.CreatedDate)
                .Select(x => new EkbWorkFlowReviewLogDto
                {
        ...
        public async Task<ResponseModel> DeleteRequestAsync(long id)
        {
            ...
            await _uow.Repository.SoftDeleteAsync<EkbServicesRequest, long>(entity);
        ...
        public async Task<ResponseModel<EkbWorkFlowGetDto>> SendBackForReviewAsync(string requestNo, string reviewNotes)
        {
            //WorkFlow'u (Akışı) Getir
            var wf = await _uow.Repository.GetQueryable<EkbWorkFlow>(x => x.RequestNo == requestNo)
                .FirstOrDefaultAsync();

            if (wf is null)
                return ResponseModel<EkbWorkFlowGetDto>.Fail("İlgili akış kaydı bulunamadı.", StatusCode.NotFound);

            if (wf.WorkFlowStatus == WorkFlowStatus.Cancelled || wf.WorkFlowStatus == WorkFlowStatus.Complated)
                return ResponseModel<EkbWorkFlowGetDto>.Fail("İptal edilmiş veya tamamlanmış akışlar geri alınamaz.", StatusCode.Conflict);
            ...
            if (servicesRequest is null)
                return ResponseModel<EkbWorkFlowGetDto>.Fail("Servis talebi bulunamadı.", StatusCode.NotFound);

            var currentStep = await _uow.Repository.GetQueryable<EkbWorkFlowStep>()
                .AsNoTracking()
                .Select(s => new { s.Id, s.Code })
                .FirstOrDefaultAsync(s => s.Id == wf.CurrentStepId);

            if (currentStep is null)
                return ResponseModel<EkbWorkFlowGetDto>.Fail("Akışın mevcut adımı bulunamadı.", StatusCode.NotFound);

            var me = await _currentUser.GetAsync();
            var meId = me?.Id ?? 0;

            var targetStep = new EkbWorkFlowStep();
            var warehouse = new EkbWarehouse();
            var technicalService = new EkbTechnicalService();
            var pricing = new EkbPricing();
            ...
            switch (currentStep.Code)
            {
                case "PRC": // Teknik Servis Adımı (TechnicalService)
                    ...
                    if (pricing != null)
                    {
                        targetStep = await _uow.Repository.GetQueryable<EkbWorkFlowStep>()
                          .AsNoTracking()
                          .FirstOrDefaultAsync(s => s.Code == "TS");
                        if (targetStep is null)
                            return ResponseModel<EkbWorkFlowGetDto>.Fail("Hedef iş akışı adımı (TS) tanımlı değil.", StatusCode.BadRequest);
                        ...
                        if (technicalService is null)
                            return ResponseModel<EkbWorkFlowGetDto>.Fail("Hedef iş akışı Teknik Servis tanımlı değil.", StatusCode.BadRequest);
                ...
                case "TS": // Teknik Servis Adımı (TechnicalService)
                    ...
                    if (technicalService != null)
                    {
                       ...
                       .FirstOrDefaultAsync(s => s.Code == "SR");
                        if (targetStep is null)
                            return ResponseModel<EkbWorkFlowGetDto>.Fail("Hedef iş akışı adımı (SR) tanımlı değil.", StatusCode.BadRequest);
                ...
                case "APR": // Kontrol ve Son Onay → Fiyatlama
                    targetStep = await _uow.Repository.GetQueryable<EkbWorkFlowStep>()
                        ...
                        .FirstOrDefaultAsync(s => s.Code == "PRC");
                    if (targetStep is null)
                        return ResponseModel<EkbWorkFlowGetDto>.Fail("Hedef iş akışı adımı (PRC) tanımlı değil.", StatusCode.BadRequest);

                    pricing = await _uow.Repository.GetQueryable<EkbPricing>()
                        .FirstOrDefaultAsync(x => x.RequestNo == requestNo);
                    if (pricing is null)
                        return ResponseModel<EkbWorkFlowGetDto>.Fail("Hedef iş akışı Fiyatlama kaydı bulunamadı.", StatusCode.BadRequest);
                ...
                case "CAPR": // Servis Talebi Adımı (ServicesRequest)
                    ...
                    if (customerForm != null)
                    {
                        ...
                        .FirstOrDefaultAsync(s => s.Code == "APR");
                        if (targetStep is null)
                            return ResponseModel<EkbWorkFlowGetDto>.Fail("Hedef iş akışı adımı (APR) tanımlı değil.", StatusCode.BadRequest);
                        customerForm.UpdatedDate = DateTime.Now;
                        customerForm.UpdatedUser = meId;
                        customerForm.Status = EkbCustomerFormStatus.AwaitingReview;
                        _uow.Repository.Update(customerForm);


                        var approval = await _uow.Repository.GetQueryable<EkbFinalApproval>().FirstOrDefaultAsync(x => x.RequestNo == requestNo);
                        if (approval is null)
                            return ResponseModel<EkbWorkFlowGetDto>.Fail("Hedef iş akışı (APR) bulunamadı", StatusCode.BadRequest);
            ...
            if (targetStep.Code is null)
                return ResponseModel<EkbWorkFlowGetDto>.Fail("Herhangi bir işlem yapılamadı.", StatusCode.BadRequest);
            ...
            var reviewLog = new EkbWorkFlowReviewLog
            {
                EkbWorkFlowId = wf.Id,
                RequestNo = requestNo,
            ...
            return ResponseModel<EkbWorkFlowGetDto>.Success(
                wf.Adapt<EkbWorkFlowGetDto>(_config)
            );
        }

        // -------------------- Warehouse --------------------
        public async Task<ResponseModel<EkbWarehouseGetDto>> GetWarehouseByIdAsync(long id)
        {
            var qWarehouse = _uow.Repository.GetQueryable<EkbWarehouse>().AsNoTracking();
            var qWorkFlow = _uow.Repository.GetQueryable<EkbWorkFlow>().AsNoTracking().Where(w => !w.IsDeleted);
            var qServices = _uow.Repository.GetQueryable<EkbServicesRequest>().AsNoTracking();
            ...
            if (dto is null)
                return ResponseModel<EkbWarehouseGetDto>.Fail("Kayıt bulunamadı.", StatusCode.NotFound);
            ...
            dto.ReviewLogs = await _uow.Repository
                .GetQueryable<EkbWorkFlowReviewLog>(x =>
                    x.RequestNo == dto.RequestNo &&
            ...
            return ResponseModel<EkbWarehouseGetDto>.Success(dto);
        }
        public async Task<ResponseModel<EkbWarehouseGetDto>> GetWarehouseByRequestNoAsync(string requestNo)
        {
            var qWarehouse = _uow.Repository.GetQueryable<EkbWarehouse>().AsNoTracking();
            var qWorkFlow = _uow.Repository.GetQueryable<EkbWorkFlow>().AsNoTracking().Where(w => !w.IsDeleted);
            var qServices = _uow.Repository.GetQueryable<EkbServicesRequest>().AsNoTracking();
        ...
        // -------------------- Teknical Services --------------------
        public async Task<ResponseModel<EkbTechnicalServiceGetDto>> GetTechnicalServiceByRequestNoAsync(string requestNo)
        {
            var query = _uow.Repository.GetQueryable<EkbTechnicalService>();
            ...
            var entity = await query
                     ...
                     .AsSplitQuery()
                     .Include(x => x.EkbServiceRequestFormImages)
                     .Include(x => x.EkbServicesImages)
                     .Include(x => x.ServiceType)
                     .FirstOrDefaultAsync();
            ...
            var dto = entity.Adapt<EkbTechnicalServiceGetDto>(_config);

            if (dto is null)
                return ResponseModel<EkbTechnicalServiceGetDto>.Fail("Kayıt bulunamadı.", StatusCode.NotFound);

            dto.ServiceRequestFormImages = entity.EkbServiceRequestFormImages
                .Select(img => img.Adapt<EkbTechnicalServiceFormImageGetDto>(_config))
                .ToList();
            dto.ServicesImages = entity.EkbServicesImages
                .Select(img => img.Adapt<EkbTechnicalServiceImageGetDto>(_config))
                .ToList();
            ...
            dto.ReviewLogs = await _uow.Repository
                ...
                .AsNoTracking()
                .OrderByDescending(x => x.CreatedDate)
                .ProjectToType<EkbWorkFlowReviewLogDto>(_config)
                .ToListAsync();
            ...
            try
            {
                ...
                if (type == TechnicalServiceImageType.Service)
                {
                    var image = await _uow.Repository
                        .GetQueryable<EkbTechnicalServiceImage>()
                    ...
                    await _uow.Repository
                        .HardDeleteAsync<EkbTechnicalServiceImage, long>(image);
                }
                else
                {
                    var image = await _uow.Repository
                        .GetQueryable<EkbTechnicalServiceFormImage>()
                        .FirstOrDefaultAsync(x => x.Id == id, cancellationToken);
                    ...
                    await _uow.Repository
                        .HardDeleteAsync<EkbTechnicalServiceFormImage, long>(image);
                }
        ...
        /// ------------------ Pricing -----------------------------------
        public async Task<ResponseModel<EkbPricingGetDto>> GetPricingByRequestNoAsync(string requestNo)
        {
            var qPricing = _uow.Repository.GetQueryable<EkbPricing>().AsNoTracking();
            var qRequest = _uow.Repository.GetQueryable<EkbServicesRequest>().AsNoTracking();
            ...
            var dto = await (
                ...
                select new EkbPricingGetDto
                {
                    ...
                    // ServicesRequest
                    OracleNo = sr != null ? sr.EkbServiceTrackNo : null,
            ...
            if (dto is null)
                return ResponseModel<EkbPricingGetDto>.Fail("Kayıt bulunamadı.", StatusCode.NotFound);
            ...
            dto.Products = new List<EkbServicesRequestProductGetDto>();
            foreach (var p in productEntities)
            {
                /*
                 * Fiyat ve currency AYNI kaynaktan çözülür.
                 */
                ...
                {
                    /*
                     * Capture edilmiş kayıt varsa öncelik captured alanlarda.
                     */
                    effectivePrice = p.CapturedUnitPrice ?? effectivePriceInfo.Price;

                    /*
                     * Eski kayıtlarda CapturedCurrency boş kalmış olabilir.
                     * Böyle bir durumda effective fiyat hangi kaynaktan
                     * geliyorsa onun CurrencyCode değeri kullanılır.
                     */
                    currency =
                        !string.IsNullOrWhiteSpace(p.CapturedCurrency)
                            ? p.CapturedCurrency
                            : effectivePriceInfo.CurrencyCode;
                }
                else
                {
                    /*
                     * Capture edilmemiş kayıtta price ve currency
                ...
                /*
                 * Kesinlikle otomatik TRY atanmaz.
                 */
                if (string.IsNullOrWhiteSpace(currency))
                {
                    return ResponseModel<EkbPricingGetDto>.Fail(
                        ...
                        $"RequestNo: {requestNo}, " +
                        $"ProductId: {p.ProductId}. " +
                        $"Ürünün etkin fiyat kaynağındaki CurrencyCode alanını kontrol ediniz.",
                        StatusCode.BadRequest);
                }
                ...
                dto.Products.Add(
                    new EkbServicesRequestProductGetDto
                    {
                        Id = p.Id,
                        RequestNo = p.RequestNo,
                        ProductId = p.ProductId,
                        ...
                        IsServiceFeeProduct =
                            p.Product?.IsServiceFeeProduct,

                        ServiceFeePercentage =
                            p.Product?.ServiceFeePercentage,
            ...
            var techService = await _uow.Repository
                ...
                .Include(ts => ts.EkbServiceRequestFormImages)
                .Include(ts => ts.EkbServicesImages)
                .FirstOrDefaultAsync();
            ...
            if (techService != null)
            {
                ...
                dto.ServicesImages = techService.EkbServicesImages
                    .Select(img => new EkbTechnicalServiceImageGetDto
                    {
                        Id = img.Id,
                        EkbTechnicalServiceId = img.EkbTechnicalServiceId,
                        Url = NormalizeImageUrl(img.Url) ?? string.Empty,
                        Caption = img.Caption
                    })
                    .ToList();
                dto.ServiceRequestFormImages = techService.EkbServiceRequestFormImages
                    .Select(img => new EkbTechnicalServiceFormImageGetDto
                    {
                        Id = img.Id,
                        Url = NormalizeImageUrl(img.Url) ?? string.Empty,
                        Caption = img.Caption
            ...
            return ResponseModel<EkbPricingGetDto>.Success(dto);
        }

        //----------------------FinalApproval ---------------------------------------------------

        public async Task<ResponseModel<EkbFinalApprovalGetDto>> GetFinalApprovalByRequestNoAsync(string requestNo)
        {
            var qFinal = _uow.Repository.GetQueryable<EkbFinalApproval>().AsNoTracking();
            var qRequest = _uow.Repository.GetQueryable<EkbServicesRequest>().AsNoTracking();
            ...
            var dto = await (
                ...
                from sr in srj.DefaultIfEmpty()
                select new EkbFinalApprovalGetDto
                {
                    Id = fa.Id,
            ...
            if (dto is null)
                return ResponseModel<EkbFinalApprovalGetDto>.Fail("Kayıt bulunamadı.", StatusCode.NotFound);
            ...
            dto.Products = productEntities
                 .Select(p =>
                 {
                     ...
                     return new EkbServicesRequestProductGetDto
                     {
                         Id = p.Id,
                         RequestNo = p.RequestNo,
                         ProductId = p.ProductId,
            ...
            var qTechnicalService = _uow.Repository.GetQueryable<EkbTechnicalService>().AsNoTracking();
            ...
            {
                // Service resimleri
                if (techService.EkbServicesImages != null && techService.EkbServicesImages.Any())
                {
                    dto.ServicesImages = techService.EkbServicesImages
                        .Select(img => new EkbTechnicalServiceImageGetDto
                        {
                ...
                // Form resimleri
                if (techService.EkbServiceRequestFormImages != null && techService.EkbServiceRequestFormImages.Any())
                {
                    dto.ServiceRequestFormImages = techService.EkbServiceRequestFormImages
                        .Select(img => new EkbTechnicalServiceFormImageGetDto
                        {
            ...
            return ResponseModel<EkbFinalApprovalGetDto>.Success(dto);
        }
        public async Task<ResponseModel<EkbFinalApprovalGetDto>> GetFinalApprovalByIdAsync(long id)
        {
            var qFinal = _uow.Repository.GetQueryable<EkbFinalApproval>().AsNoTracking();
            var qRequest = _uow.Repository.GetQueryable<EkbServicesRequest>().AsNoTracking();
        ...
        public async Task<ResponseModel> SendReviewMessage(EkbCustomerReviewMessageDto dto)
        {
            try
            {
                ...
                var wf = await _uow.Repository.GetQueryable<EkbWorkFlow>()
                ...
                // 2) Adımları bul
                var fromStep = await _uow.Repository.GetQueryable<EkbWorkFlowStep>()
                    .FirstOrDefaultAsync(x => x.Code == dto.FromStepCode);

                var toStep = await _uow.Repository.GetQueryable<EkbWorkFlowStep>()
        ...
        //-----------------------Customer Agreement ---------------------------------------------------
        public async Task<ResponseModel<EkbFinalApprovalGetDto>> GetCustomerAgreementByRequestNoAsync(string requestNo, FinalApprovalStatus status = FinalApprovalStatus.CustomerApproval)
        {
            var qFinal = _uow.Repository.GetQueryable<EkbFinalApproval>().AsNoTracking();
            var qRequest = _uow.Repository.GetQueryable<EkbServicesRequest>().AsNoTracking();
            var qTechnicalService = _uow.Repository.GetQueryable<EkbTechnicalService>().AsNoTracking();
            ...
            if (dto is null)
                return ResponseModel<EkbFinalApprovalGetDto>.Fail("Bu adımda kayıt bulunamadı.", StatusCode.NotFound);
        ...
        // -------------------- WorkFlowStep --------------------
        public async Task<ResponseModel<PagedResult<EkbWorkFlowStepGetDto>>> GetStepsAsync(QueryParams q)
        {
            var query = _uow.Repository.GetQueryable<EkbWorkFlowStep>();
            if (!string.IsNullOrWhiteSpace(q.Search))
            ...
            var items = await query
                ...
                .Take(q.PageSize)
                .ProjectToType<EkbWorkFlowStepGetDto>(_config)
                .ToListAsync();

            return ResponseModel<PagedResult<EkbWorkFlowStepGetDto>>
                .Success(new PagedResult<EkbWorkFlowStepGetDto>(items, total, q.Page, q.PageSize));
        }

        public async Task<ResponseModel<EkbWorkFlowStepGetDto>> GetStepByIdAsync(long id)
        {
            var dto = await _uow.Repository.GetQueryable<EkbWorkFlowStep>()
                .Where(x => x.Id == id)
                .ProjectToType<EkbWorkFlowStepGetDto>(_config)
                .FirstOrDefaultAsync();

            if (dto is null)
                return ResponseModel<EkbWorkFlowStepGetDto>.Fail("Kayıt bulunamadı.", StatusCode.NotFound);

            return ResponseModel<EkbWorkFlowStepGetDto>.Success(dto);
        }

        public async Task<ResponseModel<EkbWorkFlowStepGetDto>> CreateStepAsync(EkbWorkFlowStepCreateDto dto)
        {
            var entity = dto.Adapt<EkbWorkFlowStep>(_config);
            await _uow.Repository.AddAsync(entity);
            await _uow.Repository.CompleteAsync();
            return await GetStepByIdAsync(entity.Id);
        }

        public async Task<ResponseModel<EkbWorkFlowStepGetDto>> UpdateStepAsync(EkbWorkFlowStepUpdateDto dto)
        {
            var entity = await _uow.Repository.GetSingleAsync<EkbWorkFlowStep>(false, x => x.Id == dto.Id);
            if (entity is null)
                return ResponseModel<EkbWorkFlowStepGetDto>.Fail("Kayıt bulunamadı.", StatusCode.NotFound);
        ...
        public async Task<ResponseModel> DeleteStepAsync(long id)
        {
            ...
            var entity = await _uow.Repository.GetSingleAsync<EkbWorkFlowStep>(
                asNoTracking: false,
                x => x.Id == id);
            ...
            await _uow.Repository.HardDeleteAsync<EkbWorkFlowStep, long>(entity);
        ...
        // -------------------- WorkFlow (tanım) --------------------
        public async Task<ResponseModel<string>> GetRequestNoAsync(string? prefix = "EKB")
        {
            prefix ??= "EKB";
            ...
            for (int i = 0; i < 10; i++)
            {
                ...
                var query = _uow.Repository.GetQueryable<EkbWorkFlow>();
        ...
        public async Task<ResponseModel<PagedResult<EkbWorkFlowGetDto>>> GetWorkFlowsAsync(EkbWorkFlowQueryParams q)
        {
            q.Normalize(maxPageSize: 200);

            var me = await _currentUser.GetAsync();
            if (me is null)
                return ResponseModel<PagedResult<EkbWorkFlowGetDto>>.Fail(
            ...
            // Kullanıcının yetkili olduğu EKB adım kodları:
            ...
            IQueryable<EkbWorkFlow> wfBase = _uow.Repository
                .GetQueryable<EkbWorkFlow>()
                .AsNoTracking()
                .Where(w =>
                    !w.IsDeleted &&
            ...
            if (q.WorkOrderTypeIds is { Count: > 0 })
            {
                ...
                wfBase = wfBase.Where(w =>
                    serviceRequestsQuery.Any(sr =>
                        ...
                        sr.EkbServicesRequestWorkOrderTypes.Any(wot => workOrderTypeIds.Contains(wot.WorkOrderTypeId))));
            }
                ...
                var priorityAliases = new Dictionary<WorkFlowPriority, string[]>
{
...

    {
                ...
                var serviceCostStatusAliases = new Dictionary<ServicesCostStatus, string[]>
        {
            ...
            { ServicesCostStatus.Maintenance, new[] { "Bakım", "Bakim", "Bakım kapsamında", "Bakim kapsaminda", "Maintenance" } }
                ...
                qJoined = qJoined.Where(x =>
                    ...
                    // EKB Services Request
                    (
                        x.sr != null &&
                        (
                            x.sr.RequestNo.Contains(term) ||
                            (x.sr.EkbServiceTrackNo != null && x.sr.EkbServiceTrackNo.Contains(term)) ||
                    ...
                    (
                        ...
                        (
                            ...
                            (x.cf.EkbServiceTrackNo != null && x.cf.EkbServiceTrackNo.Contains(term)) ||
                    ...
                    // Tarih arama - EkbServicesRequest
                    (
                        hasDate &&
                        x.sr != null &&
            ...
            var items = await finalQuery
                .Skip((page - 1) * pageSize)
                .Take(pageSize)
                .Select(x => new EkbWorkFlowGetDto
                {
                    Id = x.wf.Id,
            ...
            return ResponseModel<PagedResult<EkbWorkFlowGetDto>>
                .Success(new PagedResult<EkbWorkFlowGetDto>(items, total, page, pageSize));
        }
        public async Task<ResponseModel> DeleteWorkFlowAsync(long id)
        {
            ...
            var meId = me?.Id ?? 0;
            // 1) Entity’yi getir (tracked olsun ki güncelleme/replace çalışsın)
            var entity = await _uow.Repository.GetSingleAsync<EkbWorkFlow>(
                asNoTracking: false,
                x => x.Id == id);
        ...
        //------------------------ Report ------------------------
        public async Task<ResponseModel<EkbWorkFlowReportDto>> GetReportAsync(string requestNo)
        {
            ...
            if (wf is null)
                return ResponseModel<EkbWorkFlowReportDto>.Fail("Akış bulunamadı.", StatusCode.NotFound);

            var dto = new EkbWorkFlowReportDto
            {
                RequestNo = requestNo,
            ...
            var sr = await _uow.Repository.GetQueryable<EkbServicesRequest>()
                .AsNoTracking()
                .Include(x => x.ServiceType)
                .Include(x => x.Customer)
                    .ThenInclude(c => c.CustomerGroup)
            ...
            if (sr is not null)
            {
                dto.ServiceRequest = new ServiceRequestSectionDto
                {
                    ...
                    WorkOrderTypes = sr.EkbServicesRequestWorkOrderTypes
            ...
            var lines = await _uow.Repository.GetQueryable<EkbServicesRequestProduct>()
                .AsNoTracking()
                .Include(p => p.Product)
                .Include(p => p.Customer)
                    .ThenInclude(c => c.Tenant)                     // 🆕
            ...
            var ts = await _uow.Repository.GetQueryable<EkbTechnicalService>()
                .AsNoTracking()
                .Include(t => t.EkbServicesImages)
                .Include(t => t.EkbServiceRequestFormImages)
            ...
            if (ts is not null)
            {
                dto.TechnicalService = new TechnicalServiceSectionDto
                {
                    ...
                    ServiceImages = ts.EkbServicesImages.Select(i => new ImageDto { Id = i.Id, Url = i.Url, Caption = i.Caption }).ToList(),
                    FormImages = ts.EkbServiceRequestFormImages.Select(i => new ImageDto { Id = i.Id, Url = i.Url, Caption = i.Caption }).ToList()
                };
            }

            // 5) Warehouse
            var wh = await _uow.Repository.GetQueryable<EkbWarehouse>()
                .AsNoTracking()
            ...
            // 6) Pricing
            var pr = await _uow.Repository.GetQueryable<EkbPricing>()
                .AsNoTracking()
            ...
            // 7) FinalApproval
            var fa = await _uow.Repository.GetQueryable<EkbFinalApproval>()
                .AsNoTracking()
            ...
            // 8) Review Logs
            dto.ReviewLogs = await _uow.Repository.GetQueryable<EkbWorkFlowReviewLog>()
                .AsNoTracking()
            ...
            return ResponseModel<EkbWorkFlowReportDto>.Success(dto);
        }
        public async Task<PagedResult<EkbWorkFlowReportListItemDto>> GetReportsAsync(EkbReportQueryParams q)
        {
            ...
            try
            {
                ...
                var list = new List<EkbWorkFlowReportListItemDto>();
                int total = 0;
                ...
                foreach (var r in rows)
                {
                    ...
                    list.Add(new EkbWorkFlowReportListItemDto
                    {
                ...
                return new PagedResult<EkbWorkFlowReportListItemDto>(list, total, q.Page, q.PageSize);
            }
            finally
            {
        ...
        public async Task<PagedResult<EkbWorkFlowReportLineDto>> GetReportLinesAsync(EkbReportQueryParams q)
        {
            // Güvenli bound
            q.Normalize(500);
            ...
            try
            {
                ...
                // Çağrı: yeni SP adı
                var rows = await conn.QueryAsync<EkbReportLineRowDto>(new CommandDefinition(
                    "ekb.usp_ReportSearch_LinesEkb",
                    p,
                    transaction: efTx,
                    commandType: CommandType.StoredProcedure,
                    commandTimeout: 60
                ...
                var list = new List<EkbWorkFlowReportLineDto>();
                int total = 0;
                ...
                foreach (var r in rows)
                {
                    ...
                    list.Add(new EkbWorkFlowReportLineDto
                    {
                        RequestNo = r.RequestNo,
                        City = r.City,
                        CustomerName = r.CustomerName,
                ...
                return new PagedResult<EkbWorkFlowReportLineDto>(list, total, q.Page, q.PageSize);
            }
            finally
            {
                if (mustClose && conn.State == ConnectionState.Open)
        ...
        public async Task<ResponseModel<PagedResult<EkbBasicReportListDto>>> GetEkbBasicWorkFlowReportAsync(EkbBasicReportQueryParams q)
        {
            try
            {
                q ??= new EkbBasicReportQueryParams();
                q.Normalize(maxPageSize: 200);
                    ...
                    wfQuery = wfQuery.Where(w =>
                        ...
                        customerFormQuery.Any(cf =>
                            ...
                            (
                                ...
                                (cf.EkbServiceTrackNo != null && cf.EkbServiceTrackNo.Contains(term)) ||
                        ...
                        srQuery.Any(sr =>
                            ...
                            (
                                ...
                                (sr.EkbServiceTrackNo != null && sr.EkbServiceTrackNo.Contains(term)) ||
                ...
                if (!string.IsNullOrWhiteSpace(q.EkbServiceTrackNo))
                {
                    wfQuery = wfQuery.Where(w =>
                        customerFormQuery.Any(cf =>
                            cf.RequestNo == w.RequestNo &&
                            cf.EkbServiceTrackNo != null &&
                            cf.EkbServiceTrackNo.Contains(q.EkbServiceTrackNo)) ||

                        srQuery.Any(sr =>
                            sr.RequestNo == w.RequestNo &&
                            sr.EkbServiceTrackNo != null &&
                            sr.EkbServiceTrackNo.Contains(q.EkbServiceTrackNo))
                    );
                }
                ...
                if (q.WorkOrderTypeIds is { Count: > 0 })
                {
                    ...
                    wfQuery = wfQuery.Where(w =>
                        srQuery.Any(sr =>
                            sr.RequestNo == w.RequestNo &&
                            sr.EkbServicesRequestWorkOrderTypes.Any(wot =>
                ...
                {
                    return ResponseModel<PagedResult<EkbBasicReportListDto>>.Success(
                        new PagedResult<EkbBasicReportListDto>(
                            new List<EkbBasicReportListDto>(),
                            total,
                            q.Page,
                            q.PageSize
                        )
                ...
                var ekbWorkOrderTypes = await _uow.Repository
                    .GetQueryable<EkbServicesRequestWorkOrderType>()
                    .AsNoTracking()
                    .Where(x => requestNos.Contains(x.EkbServicesRequest.RequestNo))
                    .Select(x => new
                    {
                        RequestNo = x.EkbServicesRequest.RequestNo,
                        x.WorkOrderType.Id,
                ...
                var ekbWotDict = ekbWorkOrderTypes
                    .GroupBy(x => x.RequestNo)
                    .ToDictionary(
                        x => x.Key,
                ...
                var items = workflows.Select(w =>
                {
                    ...
                    return new EkbBasicReportListDto
                    {
                        WorkFlowId = w.Id,
                        ...
                        EkbServiceTrackNo =
                            !string.IsNullOrWhiteSpace(sr?.EkbServiceTrackNo)
                                ? sr.EkbServiceTrackNo
                                : cf?.EkbServiceTrackNo,
                        ...
                        WorkOrderTypes = ekbWotDict.TryGetValue(w.RequestNo, out var ekbWotList)
                            ? ekbWotList
                            : new List<WorkOrderTypeLiteDto>(),
                        Products = productDict.TryGetValue(w.RequestNo, out var productList)
                            ? productList
                            : new List<EkbServicesRequestProductGetDto>()
                    };
                }).ToList();
            ...
            {
                _logger.LogError(ex, "GetEkbBasicWorkFlowReportAsync");

                return ResponseModel<PagedResult<EkbBasicReportListDto>>.Fail(
                    $"EKB workflow raporu getirilirken hata oluştu: {ex.Message}",
                    StatusCode.Error
                );
            }
        }
        public async Task<(byte[] Content, string FileName, string ContentType)> ExportEkbBasicWorkFlowReportAsync(EkbBasicReportQueryParams q)
        {
            q ??= new EkbBasicReportQueryParams();
            ...
            try
            {
                ...
                while (true)
                {
                    ...
                    var result = await GetEkbBasicWorkFlowReportAsync(q);

                    if (result.Data is null)
                    {
                        throw new InvalidOperationException(
                            "EKB temel rapor verisi export için alınamadı.");
                    }
                ...
                var fileName = $"EKB_Temel_Rapor_{DateTime.Now:yyyyMMdd_HHmm}.xlsx";
            ...
            {
                _logger.LogError(ex, "ExportEkbBasicWorkFlowReportAsync");
        ...
        private static void WriteBasicReportRow(IXLWorksheet ws, int row, int sequenceNo, EkbBasicReportListDto x)
        {
            var c = 1;
            ...
            ws.Cell(row, c++).Value = x.EkbServiceTrackNo ?? string.Empty;
            ...
            SetDecimal(ws.Cell(row, c++), x.PricingTotalAmountEur, "#,##0.00");
        ...
        //excel export 
        public async Task<(byte[] Content, string FileName, string ContentType)> ExportReportLinesAsync(EkbReportQueryParams q)
        {
            ...
            try
            {
                ...
                foreach (var x in rows)
                {
                    ...
                    var uTL = ws.Cell(r, c++);
                    uTL.Value = x.LineUnitPriceTL;
                    uTL.Style.NumberFormat.Format = "#,##0.00";

                    var tTL = ws.Cell(r, c++);
                    tTL.Value = x.LineTotalTL;
                    tTL.Style.NumberFormat.Format = "#,##0.00";

                    var uUS = ws.Cell(r, c++);
                    uUS.Value = x.LineUnitPriceUSD;
                    uUS.Style.NumberFormat.Format = "#,##0.00";

                    var tUS = ws.Cell(r, c++);
                    tUS.Value = x.LineTotalUSD;
                    tUS.Style.NumberFormat.Format = "#,##0.00";
                    ...
                    disc.Value = x.DiscountPercent / 100m;
        ...
        //Arşiv 
        public async Task<ResponseModel<PagedResult<EkbWorkFlowArchiveListDto>>> GetArchiveListAsync(EkbWorkFlowArchiveFilterDto filter)
        {
            try
            {
                // The archive query and product subquery must share the same context instance.
                var q = _ctx
                    .Set<EkbWorkFlowArchive>()
                    .AsNoTracking();
                ...
                var list = new List<EkbWorkFlowArchiveListDto>(pageRows.Count);
                ...
                foreach (var a in pageRows)
                {
                    ...
                    try
                    {
                        var wf = JsonConvert.DeserializeObject<EkbWorkFlow>(a.EkbWorkFlowJson);
                    ...
                    list.Add(new EkbWorkFlowArchiveListDto
                    {
                        Id = a.Id,
                ...
                // --- Sonuç ---
                var paged = new PagedResult<EkbWorkFlowArchiveListDto>(
                    Items: list,
                ...
                return ResponseModel<PagedResult<EkbWorkFlowArchiveListDto>>.Success(paged);
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "GetArchiveListAsync");
                return ResponseModel<PagedResult<EkbWorkFlowArchiveListDto>>.Fail(
        ...
        private IQueryable<EkbWorkFlowArchive> ApplyArchiveTextFilters(IQueryable<EkbWorkFlowArchive> query, EkbWorkFlowArchiveFilterDto filter)
        {
            ...
            if (!string.IsNullOrWhiteSpace(filter.SearchText))
            {
                ...
                var matchingProductArchives = ArchiveProductSearch.MatchingArchiveIds<EkbWorkFlowArchive>(_ctx, pattern);
                query = query.Where(x => EF.Functions.Like(x.RequestNo, pattern) || EF.Functions.Like(x.ArchiveReason, pattern) ||
                    ...
                    EF.Functions.Like(x.EkbWorkFlowJson, pattern) || EF.Functions.Like(x.EkbServicesRequestJson, pattern) || EF.Functions.Like(x.EkbServicesRequestProductsJson, pattern) ||
                    EF.Functions.Like(x.EkbWorkFlowReviewLogsJson, pattern) || EF.Functions.Like(x.EkbTechnicalServiceJson, pattern) || EF.Functions.Like(x.EkbWarehouseJson, pattern) ||
                    EF.Functions.Like(x.EkbPricingJson, pattern) || EF.Functions.Like(x.EkbFinalApprovalJson, pattern) ||
                    (x.EkbWorkflowAttachmentsJson != null && EF.Functions.Like(x.EkbWorkflowAttachmentsJson, pattern)) ||
                    matchingProductArchives.Contains(x.Id));
            }
            return query;
        }

        public async Task<ResponseModel<EkbWorkFlowArchiveDetailDto>> GetArchiveDetailByIdAsync(long id)
        {
            try
            {
                var archive = await _uow.Repository
                    .GetQueryable<EkbWorkFlowArchive>()
                    .AsNoTracking()
                    .FirstOrDefaultAsync(x => x.Id == id);

                if (archive is null)
                {
                    return ResponseModel<EkbWorkFlowArchiveDetailDto>.Fail(
                ...
                return ResponseModel<EkbWorkFlowArchiveDetailDto>.Success(dto);
            }
            catch (Exception ex)
            {
        ...
        public async Task<ResponseModel<EkbWorkFlowArchiveDetailDto>> GetArchiveDetailByRequestNoAsync(string requestNo)
        {
            try
            {
        ...
        private async Task ArchiveWorkflowAsync(string requestNo, string archiveReason, CancellationToken ct = default)
        {
            ...
            var serviceImages = await _uow.Repository
                .GetQueryable<EkbTechnicalServiceImage>()
                .AsNoTracking()
                .Where(x => x.EkbTechnicalServiceId == technicalService.Id)
                .ToListAsync(ct);
            ...
            var workflowAttachments = await _uow.Repository
                .GetQueryable<EkbWorkflowAttachment>()
                .AsNoTracking()
                .Where(x => x.RequestNo == requestNo)
                .ToListAsync(ct);
            ...
            var archive = new EkbWorkFlowArchive
            {
                RequestNo = requestNo,
                ArchivedAt = DateTime.Now,
                ArchiveReason = archiveReason,
                EkbServicesRequestJson = servicesRequestJson,
                EkbServicesRequestProductsJson = productsJson,
                CustomerJson = customerJson,
                ApproverTechnicianJson = approverTechnicianJson,
                CustomerApproverJson = customerApproverJson,
                EkbWorkFlowJson = workFlowJson,
                EkbWorkFlowReviewLogsJson = reviewLogsJson,
                EkbTechnicalServiceJson = technicalServiceJson,
                EkbTechnicalServiceImagesJson = techServiceImagesJson,
                EkbTechnicalServiceFormImagesJson = techServiceFormImagesJson,
                EkbWarehouseJson = warehouseJson,
                EkbPricingJson = pricingJson,
                EkbFinalApprovalJson = finalApprovalJson,
                EkbWorkflowAttachmentsJson = workflowAttachmentsJson
            };
        ...
        private static string? GetTechnicianEmail(EkbWorkFlow wf)
        {
        ...
        private async Task<List<string>> GetUserStepsByMenuPermission(long userId)
        {
            ...
            {
                ["EkbServiceRequestWarehouse"] = "WH",
                ["EkbServiceRequestPricing"] = "PRC",
                ["EkbCancelledFlows"] = "CNC",
                ["EkbCustomerServiceRequestCreate"] = "CF",
                ["EkbServiceRequestCustomerAgreement"] = "CAPR",
                ["EkbServiceRequestFinalApproval"] = "APR",
                ["EkbServiceRequestCreate"] = "SR",
                ["EkbServiceRequestComplate"] = "CMP",
                ["EkbServiceRequestTechnicalService"] = "TS",
            };
        ...
        private async Task<EkbWorkFlowArchiveDetailDto> BuildArchiveDetailDtoAsync(EkbWorkFlowArchive archive)
        {
            EkbServicesRequest? servicesRequest = null;
            List<EkbServicesRequestProduct> products = new();
            Customer? customer = null;
            User? approverTechnician = null;
            ProgressApprover? customerApprover = null;
            EkbWorkFlow? wf = null;
            List<EkbWorkFlowReviewLog> reviewLogs = new();
            EkbTechnicalService? technicalService = null;
            List<ArchiveImageDto> serviceImages = new();
            List<ArchiveImageDto> formImages = new();
            EkbWarehouse? warehouse = null;
            EkbPricing? pricing = null;
            EkbFinalApproval? finalApproval = null;
            List<EkbWorkflowAttachment> workflowAttachments = new();

            try { servicesRequest = JsonConvert.DeserializeObject<EkbServicesRequest>(archive.EkbServicesRequestJson); } catch { }
            try { products = JsonConvert.DeserializeObject<List<EkbServicesRequestProduct>>(archive.EkbServicesRequestProductsJson) ?? new(); } catch { }
            ...
            try { wf = JsonConvert.DeserializeObject<EkbWorkFlow>(archive.EkbWorkFlowJson); } catch { }
            try { reviewLogs = JsonConvert.DeserializeObject<List<EkbWorkFlowReviewLog>>(archive.EkbWorkFlowReviewLogsJson) ?? new(); } catch { }
            try { technicalService = JsonConvert.DeserializeObject<EkbTechnicalService>(archive.EkbTechnicalServiceJson); } catch { }
            try { serviceImages = JsonConvert.DeserializeObject<List<ArchiveImageDto>>(archive.EkbTechnicalServiceImagesJson) ?? new(); } catch { }
            try { formImages = JsonConvert.DeserializeObject<List<ArchiveImageDto>>(archive.EkbTechnicalServiceFormImagesJson) ?? new(); } catch { }
            try { warehouse = JsonConvert.DeserializeObject<EkbWarehouse>(archive.EkbWarehouseJson); } catch { }
            try { pricing = JsonConvert.DeserializeObject<EkbPricing>(archive.EkbPricingJson); } catch { }
            try { finalApproval = JsonConvert.DeserializeObject<EkbFinalApproval>(archive.EkbFinalApprovalJson); } catch { }
            try { workflowAttachments = JsonConvert.DeserializeObject<List<EkbWorkflowAttachment>>(archive.EkbWorkflowAttachmentsJson) ?? new(); } catch { workflowAttachments = new(); }
            ...
            var attachmentDtos = workflowAttachments
              .Select(x => new EkbWorkflowAttachmentGetDto
              {
                  Id = x.Id,
                  RequestNo = x.RequestNo,
            ...
            var snapshot = new EkbWorkFlowArchiveSnapshotDto
            {
            ...
            return new EkbWorkFlowArchiveDetailDto
            {
                Id = archive.Id,
        ...
        private async Task<ResponseModel> EnsurePricesCapturedFromDtoAsync_(string requestNo, IEnumerable<EkbServicesRequestProductCreateDto>? productsDto)
        {
            var dtoDict = (productsDto ??
                           Enumerable.Empty<EkbServicesRequestProductCreateDto>())
            ...
            foreach (var product in list)
            {
                ...
                var currency = string.IsNullOrWhiteSpace(product.Product?.PriceCurrency) ? "TRY" : product.Product.PriceCurrency.Trim().ToUpperInvariant();
        ...
        private async Task<ResponseModel> EnsurePricesCapturedFromDtoAsync(string requestNo, IEnumerable<EkbServicesRequestProductCreateDto>? productsDto)
        {
            ...
            var dtoDict = (productsDto ??
                           Enumerable.Empty<EkbServicesRequestProductCreateDto>())
                .GroupBy(x => x.ProductId).ToDictionary(
                    x => x.Key,
                    x => x.First());
            ...
            // -------------------------------------------------------
            // Talebin Tenant bilgisini bul
            // RequestNo -> ServicesRequest -> Customer -> TenantId
            // -------------------------------------------------------

            var tenantId = await _uow.Repository
                .GetQueryable<EkbServicesRequest>()
                .AsNoTracking()
                .Where(x =>
            ...
            // -------------------------------------------------------
            // Product.PriceCurrency olmayan ürünleri belirle
            // -------------------------------------------------------

            var productsWithoutCurrency = list
                .Where(x =>
                    dtoDict.ContainsKey(x.ProductId) &&
                    string.IsNullOrWhiteSpace(x.Product?.PriceCurrency))
                .Select(x => x.ProductId)
                .Distinct()
                .ToList();
            ...
            // -------------------------------------------------------
            // TenantProductPrice currency bilgilerini TEK SORGU ile al
            // -------------------------------------------------------

            var tenantCurrencyByProductId = new Dictionary<long, string>();

            if (productsWithoutCurrency.Count > 0 && tenantId.HasValue && tenantId.Value > 0)
            {
                var tenantPrices = await _uow.Repository
                    .GetQueryable<TenantProductPrice>()
                    .AsNoTracking()
                    .Where(x =>
                        !x.IsDeleted &&
                        x.TenantId == tenantId.Value &&
                        productsWithoutCurrency.Contains(x.ProductId))
                    .Select(x => new
                    {
                        x.ProductId,
                        x.CurrencyCode
                ...
                tenantCurrencyByProductId = tenantPrices
                    .Where(x => !string.IsNullOrWhiteSpace(x.CurrencyCode))
                    .GroupBy(x => x.ProductId)
                    .ToDictionary(
                        x => x.Key,
                        x => NormalizeCurrency(
                            x.First().CurrencyCode)!
                    );
            }


            // -------------------------------------------------------
            // Her ürünün kullanılacak currency bilgisini bir kez çöz
            // -------------------------------------------------------

            var resolvedCurrencyByProductId =
                new Dictionary<long, string>();

            foreach (var product in list)
            {
                if (!dtoDict.ContainsKey(product.ProductId))
                    continue;

                // 1. Öncelik Product.PriceCurrency
                var currency = NormalizeCurrency(product.Product?.PriceCurrency);

                // 2. Product currency yoksa TenantProductPrice.CurrencyCode
                if (currency is null &&
                    tenantCurrencyByProductId.TryGetValue(
                        product.ProductId,
                        out var tenantCurrency))
                {
                    currency = tenantCurrency;
                }

                // Currency hâlâ bulunamadıysa TRY varsayma.
                if (string.IsNullOrWhiteSpace(currency))
                {
                    return ResponseModel.Fail(
                        $"Ürün para birimi belirlenemedi. " +
                ...
                resolvedCurrencyByProductId[product.ProductId] = currency;
            }


            // -------------------------------------------------------
            // Hizmet bedeli olmayan ürünlerin toplamları
            // Currency bazında hesaplanır.
            // -------------------------------------------------------
            ...
            foreach (var product in list)
            {
                ...
                // Hizmet bedeli ürünü başka hizmet bedelinin
                // matrahına girmez.
                if (product.Product?.IsServiceFeeProduct == true)
                    continue;

                var currency =
                    resolvedCurrencyByProductId[product.ProductId];

                // Normal ürün fiyatı mevcut davranıştaki gibi DTO'dan.
                var unitPrice = dtoItem.Price ?? 0m;
            ...
            foreach (var product in list)
            {
                ...
                // Artık Product.PriceCurrency okumuyoruz.
                // Yukarıda tek bir noktada çözülmüş currency kullanılıyor.
                var currency =
                    resolvedCurrencyByProductId[product.ProductId];
                ...
                if (product.Product?.IsServiceFeeProduct == true)
                {
                     ...
                     * Hizmet Bedeli Birim Fiyatı =
                     *
                     * Aynı para birimindeki normal ürün toplamı
                     * × Hizmet bedeli yüzdesi
                     * ÷ 100
                     */
                    unitPrice = Math.Round(
                ...
                {
                    // Normal ürün fiyatı DTO'dan gelir.
                    unitPrice = dtoItem.Price;
                }
                ...
                /*
                 * Burada CapturedSource'u Tenant yapmak doğru olmaz.
                 *
                 * Çünkü yalnızca currency TenantProductPrice'tan gelmiş olabilir.
                 * Birim fiyat hâlâ dtoItem.Price üzerinden geliyor.
                 */
                product.CapturedSource = CapturedPriceSource.Standard;
                product.CapturedUnitPrice = unitPrice;
                product.CapturedCurrency = currency;
                product.CapturedTotal = totalPrice;
                product.CapturedAt = DateTime.Now;
        ...
        private void SyncEkbWorkOrderTypes(
            EkbServicesRequest request,
            IReadOnlyCollection<long> workOrderTypeIds)
        {
            var requestedIds = workOrderTypeIds.ToHashSet();

            var currentRelations = request.EkbServicesRequestWorkOrderTypes.ToList();
            ...
            {
                _uow.Repository.Add(new EkbServicesRequestWorkOrderType
                {
                    EkbServicesRequestId = request.Id,
        ...
        public async Task<ResponseModel<WorkingStatusDto>> StartWorking(StartWorkingDto dto)
        {
            try
            {
                ...
                var session = new EkbTechnicalServiceWorkSession
                {
                    RequestNo = dto.RequestNo,
                    WorkFlowId = wf.Id,
                    TechnicalServiceId = technicalService.Id,
        ...
        private async Task<(EkbWorkFlow wf, EkbServicesRequest request, Customer customer, EkbTechnicalService technicalService)?> GetTechnicalServiceContextAsync(string requestNo)
        {
            var wf = await _uow.Repository
                .GetQueryable<EkbWorkFlow>()
                .Include(x => x.ApproverTechnician)
        ...
        {
            return
                $"FlowAssist EKB Teknik Servis Testi [FA:{requestNo}] - " +
        ...
        private async Task<(bool Success, string? ErrorMessage)> CloseActiveWorkingSessionAsync(EkbTechnicalServiceWorkSession session, string reason)
        {
            try
            {
                var nowUtc = DateTimeOffset.UtcNow;
        ...
        private async Task<bool> IsManitouTechnicalServiceTestEnabledAsync(long? tenantId, CancellationToken cancellationToken = default)
        {
            ...
            return await _uow.Repository
                ...
                .AnyAsync(x =>
                    x.Id == tenantId.Value &&
                    x.Code == CommonConstants.ManitouTestTenantCodeEKB &&
        ...
        private async Task<WorkflowAttachmentChangeSet> ApplyWorkflowAttachmentChangesAsync(
                ...
                IEnumerable<EkbWorkflowAttachmentReplaceDto>? replacedAttachments,
                string stepCode,
                CancellationToken cancellationToken = default)
        {
            ...
            var replacements = replacedAttachments?
                ...
                .Select(x => x.First())
                .ToList() ?? new List<EkbWorkflowAttachmentReplaceDto>();
            ...
            try
            {
                ...
                foreach (var file in newFiles)
                {
                    ...
                    var entity = new EkbWorkflowAttachment
                    {
                        RequestNo = requestNo,
        ...
        private async Task<List<EkbWorkflowAttachmentGetDto>> GetWorkflowAttachmentsAsync(string requestNo, CancellationToken cancellationToken = default)
        {
        ...
        public async Task<ResponseModel<PagedResult<EkbAccountingServiceReportDto>>> GetAccountingServiceReportAsync(EkbAccountingReportQueryParams q)
        {
            try
            {
                q ??= new EkbAccountingReportQueryParams();
                q.Normalize(maxPageSize: 200);
                ...
                var accountingQuery = _uow.Repository
                    .GetQueryable<EkbAccountingProcess>()
                    .AsNoTracking()
                    .Where(x => !x.IsDeleted);
                ...
                // -------------------------------------------------------
                // SERVİS TÜRÜ
                // EkbServicesRequest.ServiceTypeId üzerinden filtrelenir.
                // -------------------------------------------------------
                ...
                if (requestNos.Count == 0)
                {
                    return ResponseModel<PagedResult<EkbAccountingServiceReportDto>>
                        .Success(
                            new PagedResult<EkbAccountingServiceReportDto>(
                                new List<EkbAccountingServiceReportDto>(),
                                total,
                                q.Page,
                                q.PageSize
                            )
                ...
                var serviceRequests = await serviceRequestQuery
                       ...
                       .Select(sr => new
                       {
                           ...
                           EkbServicesRequestWorkOrderTypes =
                                         sr.EkbServicesRequestWorkOrderTypes
                                             .Select(x => new
                                             {
                                                 Id = x.WorkOrderTypeId,
                                                 x.WorkOrderType.Name,
                ...
                var productDict = productEntities
                    ...
                    .ToDictionary(
                        ...
                        g => g.Select(p =>
                        {
                            ...
                            return new EkbAccountingProductDto
                            {
                                ProductId = p.ProductId,
                                ProductCode = p.Product?.ProductCode,
                                ProductName = p.Product?.Description,
                ...
                var items = workflows.Select(w =>
                {
                    ...
                    return new EkbAccountingServiceReportDto
                    {
                        RequestNo = w.RequestNo,
                        ...
                        ServiceType = sr?.ServiceType == null
                            ? null
                            : new EkbAccountingServiceTypeDto
                            {
                        ...
                        EkbServicesRequestWorkOrderTypes =
                            sr?.EkbServicesRequestWorkOrderTypes
                                .Select(x => new EkbAccountingWorkOrderTypeDto
                                {
                                    Id = x.Id,
                                    Name = x.Name,
                                    Code = x.Code
                                ...
                                .ToList()
                            ?? new List<EkbAccountingWorkOrderTypeDto>(),

                        Products = productDict.TryGetValue(
                            w.RequestNo,
                            out var products)
                                ? products
                                : new List<EkbAccountingProductDto>()
                    };
                }).ToList();
            ...
            catch (Exception ex)
            {
                ...
                return ResponseModel<PagedResult<EkbAccountingServiceReportDto>>
                    .Fail(
                        $"EKB muhasebe servis raporu alınırken hata oluştu: {ex.Message}",
                        StatusCode.Error
                    );
            }
        }
        public async Task<ResponseModel<EkbAccountingStatusDto>> ToggleAccountingProcessAsync(string requestNo)
        {
            try
            {
                if (string.IsNullOrWhiteSpace(requestNo))
                {
                    return ResponseModel<EkbAccountingStatusDto>.Fail(
                ...
                {
                    accounting = new EkbAccountingProcess
                    {
                        RequestNo = requestNo,
                        IsProcessed = true,
                ...
                return ResponseModel<EkbAccountingStatusDto>.Success(
                    new EkbAccountingStatusDto
                    {
        ...
        public async Task<ResponseModel<List<EkbWorkflowAttachmentGetDto>>> AddAccountingAttachmentsAsync(string requestNo, IReadOnlyCollection<IFormFile>? files, CancellationToken cancellationToken = default)
        {
            var uploadedStoredFileNames = new List<string>();
            var createdAttachments = new List<EkbWorkflowAttachment>();
            ...
            try
            {
                ...
                if (string.IsNullOrWhiteSpace(requestNo))
                {
                    return ResponseModel<
                        List<EkbWorkflowAttachmentGetDto>>.Fail(
                            "Talep numarası zorunludur.",
                            StatusCode.BadRequest);
                }
                ...
                foreach (var file in fileList)
                {
                    ...
                    var attachment = new EkbWorkflowAttachment
                    {
                        RequestNo = requestNo,
                ...
                return ResponseModel<
                    List<EkbWorkflowAttachmentGetDto>>.Success(
                        result);
            }
            catch (OperationCanceledException)
                when (cancellationToken.IsCancellationRequested)
        ...
        public async Task<ResponseModel<List<EkbWorkflowAttachmentGetDto>>> GetAccountingAttachmentsAsync(string requestNo, CancellationToken cancellationToken = default)
        {
            try
            {
                if (string.IsNullOrWhiteSpace(requestNo))
                {
                    return ResponseModel<List<EkbWorkflowAttachmentGetDto>>.Fail(
                        "Talep numarası zorunludur.",
                        StatusCode.BadRequest);
                }
                ...
                return ResponseModel<List<EkbWorkflowAttachmentGetDto>>
                    .Success(attachments);
            }
            catch (Exception ex)
            {
        ...
        public async Task<ResponseModel<List<EkbWorkflowAttachmentGetDto>>> DeleteAccountingAttachmentAsync(string requestNo, long attachmentId, CancellationToken cancellationToken = default)
        {
            try
            {
                if (string.IsNullOrWhiteSpace(requestNo))
