...
{
    public class EkbOvertimeReportService : IEkbOvertimeReportService
    {
        private readonly IUnitOfWork _uow;
        private readonly ILogger<EkbOvertimeReportService> _logger;
        private readonly ICurrentUser _currentUser;
        private readonly IWorkingHourPolicyService _workingHourPolicyService;

        public EkbOvertimeReportService(
            IUnitOfWork uow,
            ILogger<EkbOvertimeReportService> logger,
        ...
        public async Task<ResponseModel<EkbTechnicianOvertimeReportDto>> GetTechnicianOvertimeReportAsync(
            long technicianId,
            DateTime startDate,
            DateTime endDate,
            bool includeCustomerDetails = false)
        {
            try
            {
                ...
                if (technician == null)
                    return ResponseModel<EkbTechnicianOvertimeReportDto>.Fail(
                        "Teknisyen bulunamadı.",
                        StatusCode.NotFound);

                // EKB WorkFlow'ları çek
                ...
                var jobs = new List<EkbOvertimeJobDto>();
                ...
                foreach (var ts in technicalServices)
                {
                    ...
                    if (overtimeResult.TotalOvertimeHours > 0)
                    {
                        ...
                        {
                            job.EkbCustomerFormId = customer.Id;
                ...
                var report = new EkbTechnicianOvertimeReportDto
                {
                ...
                return ResponseModel<EkbTechnicianOvertimeReportDto>.Success(report);
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "EKB - GetTechnicianOvertimeReportAsync - TechnicianId: {TechnicianId}", technicianId);
                return ResponseModel<EkbTechnicianOvertimeReportDto>.Fail(
        ...
        public async Task<ResponseModel<EkbAllTechniciansOvertimeSummaryDto>> GetAllTechniciansOvertimeSummaryAsync(
            DateTime startDate,
            DateTime endDate)
        {
            try
            {
                ...
                var summary = new EkbAllTechniciansOvertimeSummaryDto
                {
                    StartDate = startDate,
                    EndDate = endDate,
                    TotalOvertimeHours = Math.Round(totalOvertimeHours, 2),
                ...
                return ResponseModel<EkbAllTechniciansOvertimeSummaryDto>.Success(summary);
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "EKB - GetAllTechniciansOvertimeSummaryAsync");
                return ResponseModel<EkbAllTechniciansOvertimeSummaryDto>.Fail(
            ...
            DateTime endDate)
        {
            try
            {
                ResponseModel<EkbTechnicianOvertimeReportDto>? singleReport = null;
                ResponseModel<EkbAllTechniciansOvertimeSummaryDto>? summaryReport = null;
                ...
                {
                    var worksheet = workbook.Worksheets.Add("EKB Fazla Mesai Raporu");
                ...
                {
                    var worksheet = workbook.Worksheets.Add("EKB Fazla Mesai Özeti");
            ...
            {
                _logger.LogError(ex, "EKB - ExportOvertimeReportToExcelAsync");
            ...
            if (actualEnd > workEndDateTime && actualStart < actualEnd)
            {
                ...
                if (afterHours > 0)
                {
                    ...
                    breakdown.Add(new OvertimeBreakdownDto
                    {
                        ...
                        Description = $"Normal mesai bitiminden ({workEnd.Value:HH:mm}) sonra"
                    });
                }
            }
