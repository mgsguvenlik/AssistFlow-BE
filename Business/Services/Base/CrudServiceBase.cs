...
namespace Business.Services.Base
{
        ...
        where TEntity : class
    {
        ...
        public virtual async Task<ResponseModel<PagedResult<TGetDto>>> GetPagedAsync_(QueryParams q)
        {
            var query = _repo.GetQueryable<TEntity>();
            var inc = IncludeExpression();
            if (inc is not null) query = inc(query);
        ...
        public virtual async Task<ResponseModel<PagedResult<TGetDto>>> GetPagedAsync__(QueryParams q)
        {
            var query = _repo.GetQueryable<TEntity>();
            var inc = IncludeExpression();
            if (inc is not null) query = inc(query);

            // 🔍 1. Search: tüm string property’lerde arama
        ...
        public virtual async Task<ResponseModel<PagedResult<TGetDto>>> GetPagedAsync___(QueryParams q)
        {
            var query = _repo.GetQueryable<TEntity>();
            var inc = IncludeExpression();
            if (inc is not null) query = inc(query);

            // 🔍 1. Search: tüm string + tarih property’lerinde arama
            if (!string.IsNullOrWhiteSpace(q.Search))
            {
                var parameter = Expression.Parameter(typeof(TEntity), "x");
                var stringProps = typeof(TEntity).GetProperties()
                ...
                if (DateTime.TryParse(q.Search, out var searchDate))
                {
                    foreach (var prop in dateProps)
                    {
                        ...
                        // member >= date.Date && member < date.Date.AddDays(1)
