...
public sealed partial class HelpdeskTicketService : IHelpdeskTicketService
{
    ...
    // A user can have both HELPDESK_AGENT and a higher-privileged role. Archive
    // restrictions apply only to agents who do not have ticket management access.
    private static bool IsArchivedAgentReadOnly(HelpdeskTicket ticket, CurrentUserDto user) => !CanManage(user) && IsAgent(user) && ticket.Status is (HelpdeskTicketStatus.Completed or HelpdeskTicketStatus.Rejected);
